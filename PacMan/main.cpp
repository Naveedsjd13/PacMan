...
void run_display_process()
{
    ...
    // 1. Compute window dimensions
    unsigned int window_width = static_cast<unsigned int>(CELL_SIZE * MAP_WIDTH * SCREEN_RESIZE);
    unsigned int window_height = static_cast<unsigned int>(CELL_SIZE * MAP_HEIGHT * SCREEN_RESIZE);

    // 2. Create the render window (Fixing the sf::VideoMode constructor)
    sf::RenderWindow window(sf::VideoMode({window_width, window_height}), "ParaMan - MPI + OpenMP");

    // Set the view using correct FloatRect constructor (left, top, width, height)
    sf::View view(sf::FloatRect(
        {0.f, 0.f}, 
        {static_cast<float>(CELL_SIZE * MAP_WIDTH), static_cast<float>(CELL_SIZE * MAP_HEIGHT)}
    ));
    ...
    while (window.isOpen())
    {
        ...
        while (FRAME_DURATION <= lag)
        {
            ...
            if (!game_won && !pacman.get_dead())
            {
                ...
                PacmanData pd{(short)pacman.get_position().x,
                              (short)pacman.get_position().y,
                              pacman.get_energizer_timer() > 0};
                mpi_send_pacman(pd);
                messages_sent++;
            ...
            if (FRAME_DURATION > lag)
            {
                ...
                std::string title = "ParaMan [MPI+OMP] | Level: " +
                                    std::to_string(1 + level) + " | Score: " + std::to_string(score);
                if (pacman.get_dead())
                    title += " | GAME OVER! Press ENTER";
                else if (game_won)
                    title += " | LEVEL COMPLETE! Press ENTER";
...
// ─────────────────────────────────────────────────────────────────────────────
//  RANK 1 — GHOST AI PROCESS (CPU + OpenMP)
// ─────────────────────────────────────────────────────────────────────────────
void run_ghost_ai_process()
{
    std::cout << "\n[Rank 1] GHOST AI PROCESS\n";
    ...
    std::cout << "[Rank 1] START received! Running with OpenMP threads: "
              << omp_get_max_threads() << "\n\n";
    ...
    while (state.running)
    {
        ...
        // CPU OpenMP updates ghost positions
